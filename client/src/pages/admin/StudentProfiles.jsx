...
import { Search, Loader2, UserCheck, GraduationCap, Phone, MapPin, School } from 'lucide-react';
...
const StudentProfiles = () => {
    ...
    const fetchProfiles = async () => {
        try {
            setLoading(true);
            const { data } = await axios.get(backendUrl + '/api/admin/student-profiles', {
                headers: { token }
            });
        ...
        <div className="space-y-6 pb-10 p-4 md:p-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex flex-col gap-1">
                    <h1 className="text-2xl md:text-3xl font-extrabold text-blue-600 tracking-tight uppercase">Completed Profiles</h1>
                    <p className="text-gray-500 text-sm font-medium">Viewing {filteredProfiles.length} verified student records.</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50 border-b border-gray-100">
                                <th className="p-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Student Full Name</th>
                                <th className="p-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Academic Details</th>
                                <th className="p-5 text-xs font-bold text-gray-400 uppercase tracking-widest">Institution</th>
                                <th className="p-5 text-xs font-bold text-gray-400 uppercase tracking-widest text-center">Contact</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {filteredProfiles.map((student, index) => (
                                <tr key={student._id || index} className="hover:bg-blue-50/30 transition-colors">
                                    <td className="p-5">
                                    ...
                                    <td className="p-5">
                                        <div className="flex flex-col gap-1">
                                            <div className="flex items-center gap-1 text-gray-700">
                                                ...
                                                <span className="text-xs font-bold uppercase">{student?.track}</span>
                                            </div>
                                    ...
                                    <td className="p-5">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <p className="text-sm font-bold text-gray-700">{student?.schoolName}</p>
                                        </div>
                                        <div className="flex items-center gap-1 text-gray-400">
                                            <MapPin size={12} />
                                            <span className="text-xs font-medium">{student?.location}</span>
                                        </div>
                                    </td>
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden">
                {filteredProfiles.map((student, index) => (
                    <div key={student._id || index} className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                        ...
                        <div className="bg-gray-50 p-4 rounded-2xl space-y-2">
                             <p className="text-xs font-bold text-gray-700 flex items-center gap-2">
                                <School size={14} className="text-blue-500" /> {student?.schoolName}
                             </p>
            ...
            {filteredProfiles.length === 0 && (
                <div className="p-20 text-center bg-white rounded-3xl border border-dashed border-gray-200 text-gray-400">
                    ...
                    <p className="font-bold uppercase tracking-widest text-xs">No records found matching your search</p>
                </div>
            )}
        </div>
    );
