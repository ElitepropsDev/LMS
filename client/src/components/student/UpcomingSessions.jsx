    ...
    <section className="pt-16 md:pt-24 pb-16 bg-white">
      <div className="max-w-6xl mx-auto px-4 md:px-7">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-8 md:mb-10 text-center md:text-left">
          <div>
            <h2 className="text-xl md:text-3xl font-black text-slate-800 tracking-tight">Explore Modules</h2>
            <p className="text-gray-500 mt-1 text-xs md:text-sm">A glimpse of modules coming soon.</p>
            <p className="text-blue-600 font-bold mt-1 text-[10px] md:text-sm uppercase tracking-wider">Curriculum by international educators</p>
          </div>
        ...
        {/* Grid: 2 columns on mobile (grid-cols-2), 3 on desktop */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
          {sessions.map((session, index) => (
            <div key={index} className="group rounded-2xl md:rounded-3xl border border-gray-100 bg-white overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col">
              
              {/* Thinner image on mobile */}
              <div className="relative h-24 md:h-44 overflow-hidden">
                ...
                <div className="absolute top-2 left-2 md:top-4 md:left-4 bg-white/90 backdrop-blur-sm px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[8px] md:text-[10px] font-bold text-blue-600 uppercase">
                  {session.tag}
                </div>
              </div>
              ...
              <div className="p-3 md:p-6 flex-1 flex flex-col">
                <div className="flex items-center gap-1.5 text-blue-500 text-[9px] md:text-xs font-semibold mb-2">
                  <span className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></span>
                  {session.date}
                </div>
                
                <h3 className="text-xs md:text-lg font-bold text-gray-800 leading-tight mb-1 md:mb-4 group-hover:text-blue-600 line-clamp-2">
                  {session.title}
                </h3>
                ...
                <div className="mt-auto pt-2 md:pt-4 border-t border-gray-50 flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[8px] md:text-[10px] text-gray-400 font-medium">Mode</span>
                    <span className="text-[9px] md:text-sm font-bold text-gray-700 truncate max-w-[60px] md:max-w-none">
                      {session.instructor}
                    </span>
                  </div>
                  <span className="text-[8px] md:text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-md font-black">
        ...
        <button className="md:hidden block w-full max-w-[200px] mx-auto mt-8 py-3 bg-blue-50 text-blue-600 font-bold rounded-xl text-xs shadow-lg shadow-blue-100">
  Join to Access Course Library
</button>
      </div>
    </section>
